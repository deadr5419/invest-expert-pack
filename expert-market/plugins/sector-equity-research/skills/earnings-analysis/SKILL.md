...
description: |
  盈利分析与模型更新技能（2026-08-17 由 earnings-analysis + model-update 合并），支持三种模式：
  - Preview 模式：业绩发布前的前瞻分析、情景假设、关键观测指标
  - Analysis 模式：业绩发布后的深度解读、Beat/Miss 分析、估计修正
  - Model Update 模式：新数据（业绩/指引/宏观/假设变更）落地后更新财务模型、重算估值、标记实质性变化
  触发词：earnings analysis、earnings preview、业绩分析、业绩前瞻、财报分析、季报解读、pre-earnings、post-earnings、Q1/Q2/Q3/Q4 results、update model、plug earnings、refresh estimates、update numbers for [company]、new guidance、revise estimates、业绩更新、更新模型、估计修正
---

# 盈利分析与模型更新（Earnings & Model Update）

## 功能说明

统一的盈利分析与模型更新技能，覆盖"业绩前前瞻 → 业绩后解读 → 模型更新"完整闭环：
- **Preview 模式**：业绩尚未发布 → 前瞻分析
- **Analysis 模式**：业绩已发布 → 深度解读
- **Model Update 模式**：新数据落地（业绩/指引/宏观/假设变更）→ 更新模型与估值（通常接在 Analysis 模式之后）

## 模式判断

1. 确认公司和报告季度
2. 搜索该季度业绩是否已发布
3. 已发布 → Analysis 模式（随后如需更新模型 → Model Update 模式）；未发布 → Preview 模式
...
**Step 5: 模型更新交接**
- 若需要把新数据落地到财务模型并重算估值 → 转入下方 **Model Update 模式**（Step 2 起）
...
## Model Update 模式（模型更新）

> 触发：业绩发布、指引变更、分析师假设修订、宏观变化（利率/汇率/商品）、事件驱动（并购/重组/新品/管理层变动）。通常由 Analysis 模式 Step 5 转入，也可单独触发。
